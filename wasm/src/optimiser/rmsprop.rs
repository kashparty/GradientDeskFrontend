...
impl RMSProp {
    ...
    pub fn new(params: &JsValue) -> RMSProp {
        let params: RMSPropParams = params.into_serde().unwrap();
    ...
    pub fn update_weights(&mut self) {
        for u in 0..self.weights.len() {
            self.weighted_averages[u] = &self.weighted_averages[u] * self.rho + &self.weight_updates[u].map(&|x| x * x) * (1.0 - self.rho);
            let mut denominator = self.weighted_averages[u].clone() + 1e-8;
            denominator.map_mut(&|x| 1.0 / x.sqrt());
            // Subtract the gradients for each weight matrix
            self.weights[u] -= &self.weight_updates[u].element_wise_multiply(denominator) * self.learning_rate;
        }
    }
